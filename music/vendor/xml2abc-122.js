//~ Revision: 122, Copyright (C) 2014-2025: Willem Vree
...
function reparr (n, v) { return new Array (n).fill (v); }       // arr = n * [v]
...
function disam (acc) { return {'double-flat':'flat-flat', 'double-sharp':'sharp-sharp'}[acc] || acc }
...
// data abstractions
//-------------------
function Measure (p, im, mv) {  // mv = vorige maat
    this.ixp = p;       // part number  
    this.ixm = im;      // measure number
    this.mdur = mv.mdur ? mv.mdur : 0;      // measure duration (nominal metre value in divisions)
    this.divs = mv.divs ? mv.divs : 1;      // number of divisions per 1/4
    this.mtr =  mv.mtr  ? mv.mtr : [4,4];   // metrum
...
function Note (dur, n) {
    ...
    this.type = ''      // xml note type (for values see Parser.note_type)
    ...
    this.xmlstf = 1     // xml staff number
    this.xmlvce = 1     // xml voice number
}

function Elem (string, stf) {
    this.tijd = 0;      // the time in XML division units
    this.str = string;  // any abc string that is not a note
    this.pos = 0;       // position in Music.voices for stable sorting
    this.xmlstf = stf;  // undefined when parameter not given
    this.chsym = 0;     // 1 if chord symbol
    this.isbar = 0;     // 1 if right bar line
}
...
Counter.prototype.prcnt = function (ip, wev) {  // print summary of all non zero counters
    for (var iv in this.counters ['note']) {
        ...
        if (this.getv ('note', iv) == 0 && !wev)    // no real notes counted in this voice
...
function Music (options) {
    ...
    this.gChSyms = [];      // [chord symbols for each voice for alle measures in a part]
    this.vnums = {};        // all used xml voice id's in a part (xml voice id's == numbers)
    this.wev = options.wev  // output empty voices
    ...
    this.rbm = options.rbm  // rebeam on the beat from the time signature
    this.repbra = 0;        // true if volta is used somewhere
    this.nvlt = options.v;  // no volta on higher voice numbers
    this.nbr = options.nbr  // do not introdude broken rhythm (for comparison purpose)
}
Music.prototype.initVoices = function (newPart) {
    this.vtimes = {}; this.voices = {}; this.lyrics = {}; this.chrdvs = {};
    for (var v in this.vnums) {
        ...
        this.chrdvs [v] = [];   // {voice: [Elem, Elem]}, Elem is a chord symbol
    }
...
Music.prototype.appendChordsym = function (v, chsym, offset_p) {
    var e = new Elem (chsym);
    e.chsym = 1;
    e.offset = offset_p;
    this.appendObj (v, e, 0);
    this.cnt.inc ('note', v);   // chordsyms counted as notes
}
...
Music.prototype.addBar = function (lbrk, m, balken, isSib) {    // linebreak, measure data, staff allocations
    ...
    for (var v in this.vnums) {
        ...
        var rbelm = new Elem (' ' + m.rline);
        rbelm.isbar = 1;                    // mark as a barline
        this.appendObj (v, rbelm, 0);       // insert current barline record at time maxtime
        [this.voices[v], this.chrdvs[v]] = sortMeasure (this.voices[v], m, balken, isSib, v); // make all times consistent
        ...
        if (!this.nbr) mkBroken (this.voices[v]);   // make broken rhythms
    }
    this.gMaten.push (this.voices);
    this.gLyrics.push (this.lyrics);
    this.gChSyms.push (this.chrdvs);
    this.tijd = this.maxtime = 0;
    this.initVoices ();
}
Music.prototype.outVoices = function (maatrecs, ip) {   // output all voices of part ip
    var divs = maatrecs.map (m => m.divs);
    var metra = maatrecs.map (m => m.mtr);
    var lyrlines, i, n, lyrs, vvmap, unitL, lvc, iv, im, measure, chords, xs, lyrstr, mis, t;
    ...
    for (iv in this.vnums) {
        if (this.cnt.getv ('note', iv) == 0 && !this.wev)       // no real notes counted in this voice
            continue;               // skip empty voices if this.wev is false (with empty voices)
        ...
        for (im = 0; im < this.gMaten.length; ++im) {
            ...
            chords = this.gChSyms [im][iv]; // retrieve overlapping chord symbols
            vn.push (outVoice (measure, chords, divs [im], unitL, this.rbm, metra [im]));
    ...
    this.gLyrics = [];
    this.gChSyms = [];
    this.cnt.prcnt (ip+1, this.wev);    // print summary of skipped items in this part
    return vvmap;
}
...
function ABCoutput (fnmext, pad, X, options) {
    ...
    this.title = 'T:Title';
    this.key = 'none';
    this.clefs = new Map ();    // clefs for all abc-voices, keep insertion order (py-compatibility)
    this.mtr = 'none'
...
ABCoutput.prototype.mkHeader = function (stfmap, partlist, midimap, vmpdct, koppen, edo) { // stfmap = [parts], part = [staves], stave = [voices]
    ...
    if (edo) hd.push ('%%MIDI temperamentequal ' + edo + '\n');
    ...
    for (vnum of this.clefs.keys ()) {  // voice order of python version (clefs == Map object)
        t =  midimap [vnum-1];
        ch = t[0]; prg = t[1]; vol = t[2]; pan = t[3]; dmap = t.slice (4);
        clef = this.clefs.get (vnum);
...
function abcdur (nx, divs, uL) {    // convert an musicXML duration d to abc units with L:1/uL
    ...
    if (nx.grace) {    // nx.type = whole, half, quarter, .. == 1,2,4, .. for {AA} and 2,4,8, .. for {A}
        [num, den] = simplify (16, nx.type || uL);
    }
...
function setKey (fifths, mode, edo) {
    var accs, key, msralts, sharpness, offTab, altplus, altmin;
    ...
    accs = ['F','C','G','D','A','E','B'];
    if (fifths >= 0) msralts = dict (accs.slice (0, fifths), reparr (fifths, 'sharp'));
    else             msralts = dict (accs.slice (fifths), reparr (-fifths, 'flat'));
    return [key, msralts];
}

function setKeyAlt ($e, parserobj) {
    var es = $e.find ('key').children ();
    var p = '';
    var msralts = {};
    var key = 'C exp ';
    es.each (function () {
        var e = $(this)
        if (e[0].tagName == 'key-step') {
            if (p) key += ' ' + p;
            p = e.text ().trim ();
        } else if (e[0].tagName == 'key-alter') {
            var alt = e.text ().trim ();
            if (alt) msralts [p] = Math.round (alt);
        } else if (e[0].tagName == 'key-accidental') {
            var txt = disam (e.text ().trim ());
            msralts [p] = txt;
...
function bpltel (nx, divs, metrum) {
    var noemer = divs;  // de tel van deze maat
    if (metrum[1] == 8) noemer = divs / 2;
    var tel = Math.floor (nx.tijd / noemer);
    if (metrum [1] == 8) {
        if (metrum [0] == 7 || metrum [0] == 5) {
            tel = [0,0,0,1,1,2,2][tel];
        } else {
            tel = Math.floor (tel / 3);
        }
    }
    return [tel, noemer];
}

function outVoice (measure, chords, divs, unitL, rebeam, metrum) { // note/elem objects of one measure in one voice
    var ix = 0, t;
    ...
    var vs = [], nospace, s, telvor = -1, durvor = 0, durstr;
    for (var i = 0; i < measure.length; ++i) {
        ...
        if (nx instanceof Note) {
            ...
            s += nx.after;
            nospace = nx.beam;
            var [tel, noemer] = bpltel (nx, divs, metrum);
            if (rebeam && nx.dur < divs && durvor < divs) { // alleen tussen korte noten kijken
                nospace = tel == telvor;        // noten binnen een tel
                if (!nospace && nx.tijd % noemer != 0)    // eerste noot in de volgende tel
            ...
            var overlay = [];
            var tvor = 0;
            for (var ch of chords) {        // translate overlapping chord symbols as ABC overlay   
...
function elemToNewVoice (nx, balken, balkvce, lastStaffChange, v, vtijd) {
    var curstf = balken [balkvce];
    if (nx.xmlstf && nx.xmlstf > 0 && nx.xmlstf != curstf) { // staff change needed
        lastStaffChange = [v.length, curstf];   // (index in v of this change element e, current staff)
        var dstaff = nx.xmlstf - curstf;    // relative new staff number
        balken [balkvce] = nx.xmlstf;       // the new staff for this voice
        var sgn = dstaff > 0 ? '+' : '';    // force a plus sign
        var e = new Elem ('[I:staff ' + sgn + dstaff + ']');  // insert a move before the note
        e.tijd = vtijd;
        v.push (e);   // insert a staff change before the note or Elem (nx)
    } else {
        lastStaffChange = [];   // reset when no change was needed for nx
    }
    v.push (nx);
    return lastStaffChange;
}

function sortMeasure (voice, m, balken, isSib, balkvce) {
    ...
    var time = 0;
    var v = [];                             // new sorted voice
    var chvce = [];                         // extra voice for chords
    var rs = [];                            // holds rests in between notes
    var ixl = 0;                            // index in v of last note
    var lastStaffChange = []                // [index in v of change, change value]
    for (var i = 0; i < voice.length; ++i) {    // establish sequentiality
        ...
        if (nx instanceof Elem) {
            if (nx.chsym && nx.offset) {    // consider as overlapping
                nx.tijd += Math.floor (nx.offset);  // offset may be a float, though divisions are not
                chvce.push (nx);            // store overlapping chord symbols for use in outvoice()
                continue;
            }
            if (nx.chsym && nx.tijd < time) {       // chord symbol overlaps a current note
                chvce.push (nx);            // as above
                continue;
            } else if (nx.tijd < time) nx.tijd = time;  // shift elems without duration to where they fit
            elemToNewVoice (nx, balken, balkvce, lastStaffChange, v, time);
            time = nx.tijd;
            continue;
        }
        balkvce = isSib ? nx.xmlvce + (100 * nx.xmlstf) : nx.xmlvce;
        if (nx.tijd < time) {                   // overlapping element
            if (nx.ns[0] == 'z') continue;      // discard overlapping rest
            var o = v [ixl];                    // last Note in voice
            if (o.tijd <= nx.tijd) {            // we can do something
                if (o.ns[0] == 'z') {           // shorten rest
                    o.dur = nx.tijd - o.tijd;
                    if (o.dur == 0) {
                        v.splice (ixl, 1);      // nothing left, remove note
                        if (lastStaffChange.length) {   // also delete possible staff change element
                            var [iv, cstf] = lastStaffChange;   // caused by the (now deleted) rest
                            v.splice (iv, 1)
                            balken [balkvce] = cstf
                        }
                    }
        ...
        lastStaffChange = elemToNewVoice (nx, balken, balkvce, lastStaffChange, v, time);
        if (nx instanceof Note) {
            ixl = v.length - 1;                 // remember index of last note
            var n = nx.ns [0];
            if (n == 'x' || n == 'z') {
    ...
    if (time == 0) {
        infof ('empty measure in part %d, measure %d, it should contain at least a rest to advance the time!', [m.ixp+1, m.ixm+1] );
        var n = new Note (m.mdur, 'z'); // probeer te repareren met volle maat rust
        var bar = v [v.length - 1];
        if (bar.str && bar.str.match (/[|:\]]/)) {  // test maatsteep (Elem)
            bar.tijd = m.mdur;        // zet de eindtijd goed
            v = v.slice (0, -1).concat ([n, bar]);
        }
    }
    return [v, chvce];
}
...
function getPartlist ($ps) {    // correct part-list (from buggy xml-software)
    function mkstop (num) {     // make proper xml-element for missing part-group
        var elm = $ ('<part-group></part-group>', xmldoc);
        elm.attr ('number', num);
        elm.attr ('type', 'stop');
        return elm;
    }
...
function Parser (options) {
    ...
    this.grcNotes = [];   // notes in current grace sequence
    ...
    this.edo = 0;         // equal division of the octave (36 or 53)
    this.altkey = 0;      // true if key defined by <key-step> elements
    ...
    this.temp = options.temp    // temperament selection
    this.musfin = options.fin;  // strict,burak,musescore,finale: 0,1,2,3
    this.autoacc = options.fin < 0; // auto detection of encoding software
    if (this.musfin < 0) this.musfin = 2
    this.edo12alt = { 'flat-flat':-2, 'flat':-1, 'natural':0, 'sharp':1, 'sharp-sharp':2 }
    this.edo53alt = { 'quarter-sharp':1, 'sharp-2':2, 'sharp-3':3, 'sharp':4, 'slash-quarter-sharp':5, 'slash-sharp':8, 
        'sharp-sharp':9, 'natural':0, 'quarter-flat':-1, 'flat-2':-2, 'flat-3':-3, 'slash-flat':-4, 'flat':-5,
        'double-slash-flat':-8, 'flat-flat':-9 }
    this.edo36alt = { 'flat-down':-4, 'flat':-3, 'flat-up':-2, 'natural-down':-1, 'natural':0, 'natural-up':1, 'sharp-down':2,
        'sharp':3, 'sharp-up':4 }
    this.edomap = { 'flat-down':-8, 'flat-up':-3, 'natural-down':-2, 'natural-up':2, 'sharp-down':3, 'sharp-up':8 }
    this.edomix = { 'flat-down':-4, 'flat-up':-2, 'natural-down':-1, 'natural-up':1, 'sharp-down':2, 'sharp-up':4 } // to be divided by /3
    this.note_type = { 'whole':1, 'half':2, 'quarter':4, 'eighth':8, '16th':16, '32nd':32, '64th':64 }
}
...
Parser.prototype.scanEdo = function ($e) {  // check for edo53
    var accs = $e.find ('accidental').toArray ();
    accs = accs.concat ($e.find ('key-accidental').toArray ());     // also scan the key
    var edo53 = false, edo36 = false;
    for (var acc of accs) {
        var x = disam ($(acc).text ());
        if (x in this.edo12alt) continue;
        if (x in this.edo36alt) edo36 = true;
        if (x in this.edo53alt) edo53 = true;
    }
    if (edo53  && edo36) infof ('this score mixes edo53 and edo36 accidentals.',[]);
    if (edo36 && this.temp == 1) { edo53 = true; edo36 = false; }   // force mapping of edo36 to edo53
    this.edo = edo53 ? 53 : (edo36 ? 36 : 0);
    if (this.temp == 2) this.edomap = this.edomix
    if (this.edo > 0) {
        var sw = $e.find ('identification>encoding>software').text ();
        if (sw && this.autoacc) {
            if (sw.search (/abc2xml/i) >= 0) this.musfin = 0;
            if (sw.search (/burak/i) >= 0) this.musfin = 1;
            if (sw.search (/finale/i) >= 0) this.musfin = 3;
        }
        var enc = 'strict,burak,musescore,finale'.split (',') [this.musfin]
        abcOut.info (`encoder: ${enc}, tuning: EDO${this.edo} `, '')
    }
}
Parser.prototype.getcuralt = function (p, v, stf, ptc) {
    var p_v_s = p + '#' + v + '#' + stf;
    if (p_v_s in this.curalts) {    // the note in this voice has been altered before
        return this.curalts [p_v_s];
    }
    return this.msralts [ptc];      // get alteration implied by the key
}
Parser.prototype.alt2acc = function (alt, edo) {
    if (/[+-]?[01]\.0/.test (alt)) {    // [+/-]1.0 => '[+/-]1', 0.0 => '0'
        alt = Math.round (alt) + '';
    }
    if (alt.indexOf ('.') >= 0) {
        if (!alt.match (/[+-]?0\.5/)) return '';    // ignore floats when not +/-0.5
        if (this.musfin > 1) return ''; // ignore microtonal alterations in finale/mscore
        return alt == '0.5' ? 'quarter-sharp' : 'quarter-flat'
    }
    for (var k of Object.keys (this.edo12alt)) {
        if (this.edo12alt [k] == alt) return k;
    }
    return ''
}
Parser.prototype.acc2abc = function (acc) {
    var num, mapped = 0;
    switch (this.edo) {
        case 53: num = this.edo53alt [acc]; break;
        case 36: num = this.edo36alt [acc]; break;
        default: num = this.edo12alt [acc]; break;
    }
    if (num == undefined) { num = this.edomap [acc]; mapped = 1; }
    if (num == undefined) {
        infof ('unknown accidental %s', [acc])
        return '='
    }
    if (num == 0) return '=';
    else if (this.edo > 0) { // 36 or 53
        if (num > 0) acc = '^' + num;
        else acc = '_' + (-num);
        if (this.temp == 2 && mapped) acc += '/3';  // fractional mapping
        if (this.edo == 36) acc = acc.replace ('3', '');    // because in edo36: ^3 == ^ and _3 == _
        return acc
    } else {
        return ['__','_','=','^','^^'][num+2]
    }
}
Parser.prototype.ntAbc = function (ptc, oct, $note, v, stf, ntrec, isTab) {  // pitch, octave -> abc notation
    oct += this.clefOct [this.curStf [v]] || 0;     // current clef-octave-change value
    var acc = disam ($note.find ('accidental').text ());    // should be the notated accidental
    ...
    var curalt;
    var p_v_s = p +'#'+ v +'#'+ stf;    // dict key == (p, v, stf)
    if (acc) {
        if (this.musfin < 3) {  // always show accidentals for finale
            if (this.altkey) curalt = this.getcuralt (p,v,stf,ptc);  // key alteration + current alteration
            else             curalt = this.curalts [p_v_s];          // only current alteration
            if (acc == curalt && !isTab) return p;      // do not show accidental (musescore logic)
        }
        this.curalts [p_v_s] = acc;
        p = this.acc2abc (acc) + p;                 // prepend the accidental
        return p
    } else if (alt == '') {
        if (this.musfin < 3) {  // finale ignores alterations
            curalt = this.getcuralt (p, v, stf, ptc)
            if (curalt) {   // no alt but previous note or the key had one -> natural!!
                //~ if (this.musfin == 1) acc = curalt; // burak encoder
                if (this.musfin == 1) acc = this.ingrace ? curalt : 'natural'; // burak encoder
                else if (this.edo12alt [curalt]) acc = 'natural';
                else if (this.musfin < 2)        acc = 'natural';    // only honor microtonal alterations in strict mode
            }
        }   // here we fall through with alt == '' (and acc == '')
    } else {
        if (this.musfin < 3) acc = this.alt2acc (alt);      // alt has a value: map it (finale ignores alterations)
    }
    if (acc !== '') {                   // now see if we really must add an accidental
        curalt = this.getcuralt (p,v,stf,ptc);
        if (acc == curalt || (acc == 'natural' && !curalt)) return p;
        ...
        var acctxt = this.acc2abc (acc);
        infof ('accidental %d added in part %d, measure %d, voice %d note %s', [acctxt, this.msr.ixp+1, this.msr.ixm+1, v+1, p] );
        this.curalts [p_v_s] = acc;
...
Parser.prototype.doNote = function ($n) {
    ...
    if (this.isSib) v += 100 * xmlstaff;    // repair bug in Sibelius
    ...
    note.type = this.note_type [$n.find ('type').text ()] || 0;
    var $grc = $n.find ('grace');
    note.grace = $grc.length > 0;
    if (note.grace && !chord) this.grcNotes.push (note);
    ...
    if (note.grace && !this.ingrace) {      // open a grace sequence
        this.ingrace = 1;
        note.before = ['{'];
        if ($grc.attr ('slash') == 'yes' && !isTab) note.before.push ('/');   // acciaccatura
    }
    var stopgrace = !note.grace && this.ingrace;
    if (stopgrace) {                    // close the grace sequence
        if (this.grcNotes.length == 1) this.grcNotes [0].type *= 2; // L == 1/8 for {A} or 1/16 for {AA..}
        this.grcNotes = [];
        this.ingrace = 0;
    ...
    else noot = this.ntAbc (p, parseInt (o), $n, v, xmlstaff, note, isTab);
    ...
    else this.msc.appendNote (v, note, noot);
...
Parser.prototype.doAttr = function ($e) {
    var teken, dvstxt, steps, fifths, ksteps, first, key, beats, unit, mtr, toct, $clef, n, sgn, line, cs, oct, voices, v, dstaff, i, clfs, j, t, lines, lns;
    ...
    ksteps = $e.find ('key>key-step').first().text();
    ...
    if (fifths || ksteps) {
        if (fifths) {
            ...
            [key, this.msralts] = setKeyAlt ($e, this);
            this.altkey = 1;   // key defined by <key-step> elements
        }
        if (first && !(steps % 12) && abcOut.key == 'none')
    ...
    clfs = $e.find ('clef');
    var stfdetails = {}
    var dtls = $e.find ('staff-details')
    for (i = 0; i < dtls.length; i++) {
        var details = dtls.eq (i);
        stfdetails [details.attr ('number') || 1] = details; // object geindiceerd met balk nummer
    };
    for (i = 0; i < clfs.length; i++) {             // a part can have multiple staves
        ...
        if (steps && sgn != 'TAB') cs += ' transpose=' + steps; // geen transpositie in tab balk
        var stfdtl = stfdetails [n];    // <staff-details> voor balk n
        if (stfdtl) {
        ...
        if (first) this.clefMap [n] = cs;       // clef goes to header (where it is mapped to voices)
            ...
            for (j = 0; j < voices.length; ++j) {
                v = voices [j];
                var elem = new Elem ('[K:' + cs + ']', n);  // n == staff of clef, for possible staff change in sortMeasure
                this.msc.appendObj (v, elem, 0);
            }
        }
    }
}
...
Parser.prototype.doDirection = function ($e, i, $es) {  // parse a musicXML direction tag
    ...
    if (t.length) {
        ...
        if (scinst.length) {
            instid = scinst.attr ('id');    // xml ID of the score-instrument
            [chn, prg, vol, pan] = this.instMid [this.msr.ixp][instid]; // midi values for the ID
            x = this.findVoice (i, $es);    // voice of the next note in the current measure
            [parm, inst] = ['program', prg - 1];
            if (abcOut.volpan > 0) this.msc.appendElem (vs, '[I:MIDI= '+ parm +' '+ inst +']');
        }
...
Parser.prototype.doHarmony = function ($e, i, $es) {    // parse a musicXMl harmony tag
    ...
    var offset = $e.find ('offset').text ();
    this.msc.appendChordsym (vt, '"' + root + alt + kind + sus + (bass && '/' + bass) + '"', offset);
}
...
Parser.prototype.emptyStaves = function ($part, $maten) {   // fill each empty staves with an invisible rest in a new voice
    function addRestToMsre (stf, vce, $maat, $nm) {
        var dvs = $part.find ('divisions').text () || '1';  // duration of the invisible rest
        var $newbck = $('<backup></backup>', eigenaar);
        var e1 = $(`<duration>${dvs}</duration>`, eigenaar);
        $newbck.append (e1);
        var $newrest = $('<note></note', eigenaar);
        $newrest.attr ('print-object', 'no');
        var e1 = $('<rest></rest>', eigenaar);
        e1.attr ('measure', 'yes');
        var e2 = $(`<duration>${dvs}</duration>`);
        var e3 = $(`<voice>${vce}</voice>`);
        var e4 = $(`<staff>${stf}</staff>`);
        $newrest.append ([e1,e2,e3,e4]);
        var $es = $maat.children ();
        for (var i = 0; i < $es.length; i++) {  // search for index of the first note
            if ($es [i].tagName == 'note') break;
        }
        $es.splice (i, 0, $newbck [0]);   // insert before i, i now points to newbck
        $es.splice (i, 0, $newrest [0]);  // newrest goes before  newbck
        $nm.append ($es)
    }
    var eigenaar = $part[0];
    if (this.vce2stf == {}) return $part;   // empty part
    var ks = Object.keys (this.vce2stf);    // voice numbers as array of strings
    var vmax = ks.reduce ((a,b)=> Math.max (+a, +b),[]);   // new voicenumbers > vmax
    var us = Object.entries (this.stfMap)   // all staves
    us = us.filter (([stf, vlst]) => vlst.length == 0);  // all empty staves
    us = us.map    (([stf, vlst]) => stf);
    if (us.length == 0) return $part;       // no empty staves
    var $np = $('<part></part>', eigenaar);   // make a new part with added empty staff
    $np.attr ('id', $part.attr ('id'));
    var $nm = $('<measure></measure>', eigenaar);
    us.forEach ((stf, i) => {       // add invisible rest to first measure
            addRestToMsre (stf, vmax + i + 1, $maten.eq (0), $nm);
            $np.append ($nm);
            $np.append ($maten.slice (1));
            this.msc.wev = true;    // show empty staves/voices
        });
    this.locStaffMap ($np);   // update s.stfMap, s.vsc2stf and s.curStf
    return $np;
}
...
Parser.prototype.addStaffMap = function (vvmap) {   // vvmap: xml voice number -> global abc voice number
    ...
    var staff_keys = Object.keys (this.stfMap).sort ((a, b) => a - b);
    for (j = 0; j < staff_keys.length; ++j) {   // scan stfMap in alphabetical (xml) staff number order
        ...
        if (locmap.length) {            // abc voice number of staff stf
            ...
            for (i = 0; i < locmap.length; ++i) {
                ...
                abcOut.clefs.set (iv, clef + clef_attr);   // add nostems when all notes of voice had no stem
            }
        }
    }
...
Parser.prototype.parse = function (xmltree) {
    ...
    for (var ip = 0; ip < parts.length; ++ip) {
        ...
        $p = this.emptyStaves ($p, $maten);  // fill empty staves with new voices containing an invisible rest
        var $maten = $p.find ('measure');
        ...
        this.maatRecs = [];         // alle Measure objecten
        this.msr = {};              // huidige Measure object
        var aantalHerhaald = 0;     // keep track of number of repititions
        var herhaalMaat = 0;        // target measure of the repitition
        var im = 0;                 // maat index
        while (im < $maten.length) {
            var $maat = $maten.eq (im);
            var herhaal = 0, lbrk = '';
            this.msr = new Measure (ip, im, this.msr); // various measure data
            ...
            this.msc.addBar (lbrk, this.msr, this.curStf, this.isSib);
            this.maatRecs.push (this.msr);
            if (herhaal == 1) {
                herhaalMaat = im;
                im += 1;
            } else if (herhaal == 2) {
        ...
        var vvmap = this.msc.outVoices (this.maatRecs, ip);
    ...
    if (Object.keys (vvmapAll).length) {
        abcOut.mkHeader (this.gStfMap, partlist, this.midiMap, this.tabVceMap, this.koppen, this.edo);
        //~ abcOut.writeall ()
...
vertaal = function (xmltree, options_parm) {   // publish in the global name space
    ...
    var options = { u:0, b:0, n:0,  // unfold repeats (1), bars per line, chars per line
                    ...
                    temp:0, s:0,    // temperament selection, shift note heads in tablature (1)
                    fin:-1,         // determines accidental rules, ignore encoder software detection, -1 == auto
                    p:'f',          // page format: scale (1.0), width, left- and right margin in cm
                    rbm:0, nbr:0,   // rebeam on the beat of the time siganture, do not introduce broken rhythms
                    wev:0 };        // with empty voices
