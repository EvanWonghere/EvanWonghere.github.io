...
- `samples/`：34 套 FluidR3 逐音采样，共 2992 个 MP3（约 60 MB），来源与授权见 `samples/NOTICE.md`。分组：钢琴（三角、明亮、酒吧）、键盘（电钢琴、FM 电钢琴、羽管键琴、爵士风琴）、色彩打击（钢片琴、颤音琴、马林巴、钟琴）、贝斯（原声、电、合成）、吉他（尼龙弦、钢弦、清音电吉他、失真电吉他）、弦乐（合奏、拨奏、小提琴、大提琴、竖琴）、铜管（小号、长号、圆号、铜管组）、木管（长笛、单簧管、中音萨克斯、双簧管）、人声与合成（合唱、暖色铺底、锯齿波主音）。练习只加载底栏所选的一套；编曲播放按声部各自加载用到的音高，切换底栏音色时一并释放。每个采样只有约 3.2 秒：风琴、弦乐、小提琴、大提琴、铜管、木管、合唱、铺底与锯齿波主音（`SUSTAINED`）在解码后挑一段首尾响度相同的平稳片段（0.8–2 秒），把结尾 0.2 秒与起点前交叉淡化后循环，直到松开或音符结束；循环起点在 ±10 ms 内对齐相位，交叉淡化按两段的相关度归一化，同相不鼓起、反相不凹陷。钢琴、吉他、竖琴、打击乐与贝斯保持自然衰减。Strudel 沙箱里 Strudel 的采样器只接受整个音色共用的 loopBegin/loopEnd，因此底栏选中持续类音色时，页面按中央 C 采样选同一段、给每个采样做同样的交叉淡化后以 WAV 传入，沙箱为未写 loop 的 `piano` 音符补上循环，使其按时值延续；代码自己写了 loop 的保持原样。音色表在 `audio.mjs` 的 `INSTRUMENTS`，编曲可用的 id 与中文名在 `arrangement-schema.mjs`，两者由测试保持一致。
...
- `arrange-player.mjs`：一个 Web Audio 时钟驱动音符、合成鼓与各视图；支持静音/独奏、音量、摇摆
  （只移动反拍八分音符）、整首或单段循环，播放中编辑会在当前位置继续。每个音高声部用自己的音色（声部设置里选），新声部按类型给默认音色（低音为电贝斯、铺底为弦乐合奏）。
...
34 套采样加载与切换取消，编曲中多个声部不同音色同时播放；和弦工坊到钢琴跟弹、刷新恢复；作业/日志与片段循环；
