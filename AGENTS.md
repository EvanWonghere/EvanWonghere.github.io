...
- The optional AI assistant (`ai.mjs`, `ai-context.mjs`, `ai.css`) is off unless `[params.musicAI]` in `hugo.toml` is enabled and configured. It is for administrators and for AI members with the `music` scope (the quiz repository's `ai_members`), as reported by the `ai_access()` RPC: `canUse()`/`onAccessChange()` gate the assistant, vibe arrangement and Strudel snippets, and members see their remaining daily requests. `isAdmin()`/`onAdminChange()` mean administrator only and gate the cloud library of saved works. When off, nothing AI-related is rendered or loaded. It receives only a copy of progress and must never save progress, grades, homework ticks or mastery; its own storage is the sessionStorage key `hive-music-ai-pending`, and supabase-js keeps the login session shared with /quiz/ and /labs/ under `sb-*-auth-token`. Its backend (`music-*` actions, `music_messages`) lives in the quiz repository.
