...
- `static/music/catalog-versions.mjs` is generated by `node tools/export-music-catalog.mjs <quiz checkout>`, which also writes the quiz repository's `supabase/functions/ai-tutor/musicCatalog.json`. Rerun it after any lesson change; `tests/music-ai.test.mjs` fails when they drift.
